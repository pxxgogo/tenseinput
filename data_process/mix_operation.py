...
def prune_data(raw_data):
    ...
    for data in raw_data:
        ...
        flag = True
        for i in range(6):
            x = emg_data[i][EMG_PADDING:]
            x = x[:EMG_SPLIT_WINDOW_SIZE]
        ...
        flag = True
        for i in range(3):
            x = acc_data[i][ACC_PADDING:]
            x = x[:ACC_SPLIT_WINDOW_SIZE]
        ...
        data_list.append([[acc_pruned_data, emg_pruned_data], tag])
    return data_list
...
def fft_process(data_list):
    ...
    for data in data_list:
        ...
        fft_data_list.append([[acc_fft_data, emg_data], tag])
